...
| Secrets | None in the image. The build-time CA is a BuildKit secret mount, confirmed absent from the image and its history. `.env` is git-ignored. `git grep` found no key-like values beyond test placeholders. |
