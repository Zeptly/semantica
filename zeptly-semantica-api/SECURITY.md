...
* The key is compared in constant time with `hmac.compare_digest`. A missing key and a wrong key get the same `401` response.
...
* FalkorDB should also run with `--requirepass` (`FALKORDB_PASSWORD`) as defence in depth. The local compose file does both: FalkorDB is not published to the host and requires a password.
...
* `Settings.__repr__` redacts both secrets. Startup logs show `<set>` instead of values. Graph errors are logged with the query template's first line and the exception type, never with query parameters.
...
* Allowed data:
  ...
  * an optional label of 1–160 characters with no control characters.
...
* There is no rate limiting. The service is expected to sit behind private networking, called by Zeptly only.
